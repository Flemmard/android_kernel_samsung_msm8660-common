...
static void send_usb_connect_notify(struct work_struct *w)
{
	...
	pr_info("[USBH] send connect type %d\n", motg->connect_type);
...
static int msm_hsusb_init_vddcx(struct msm_otg *motg, int init)
{
	...
	if (init) {
		...
		if (IS_ERR(hsusb_vddcx)) {
			pr_err("[USBH] unable to get hsusb vddcx\n");
		...
		if (ret) {
			pr_err("[USBH] unable to set the voltage "
					"for hsusb vddcx\n");
		...
		if (ret) {
			...
			USB_PHY_VDD_DIG_VOL_MIN);
			regulator_put(hsusb_vddcx);
			pr_err("[USBH] unable to enable the hsusb vddcx\n");
			return ret;
		}
		...
		if (ret) {
			pr_err("[USBH] unable to disable hsusb vddcx\n");
			return ret;
		}
...
static int msm_hsusb_ldo_init(struct msm_otg *motg, int init)
{
	...
	if (init) {
		...
		if (IS_ERR(hsusb_3p3)) {
			pr_err("[USBH] unable to get hsusb 3p3\n");
		...
		rc = regulator_set_voltage(hsusb_3p3, USB_PHY_3P3_VOL_MIN,
				USB_PHY_3P3_VOL_MAX);
		if (rc) {
			pr_err("[USBH] unable to set voltage level for"
					"hsusb 3p3\n");
			goto put_3p3;
		}

		pr_info("[USBH] %s: ldo3v3, min volt: %d, max volt: %d\n", __func__,
		...
		if (IS_ERR(hsusb_1p8)) {
			pr_err("[USBH] unable to get hsusb 1p8\n");
...
static int msm_hsusb_config_vddcx(int high)
{
	...
	if (ret) {
		pr_err("[USBH] %s: unable to set the voltage for regulator "
			"HSUSB_VDDCX\n", __func__);
		return ret;
	}

	pr_debug("[USBH] %s: min_vol:%d max_vol:%d\n", __func__, min_vol, max_vol);
...
static int msm_hsusb_ldo_enable(struct msm_otg *motg, int on)
{
	...
	if (IS_ERR(hsusb_1p8)) {
		pr_err("[USBH] %s: HSUSB_1p8 is not initialized\n", __func__);
		return -ENODEV;
	}

	if (IS_ERR(hsusb_3p3)) {
		pr_err("[USBH] %s: HSUSB_3p3 is not initialized\n", __func__);
		return -ENODEV;
	}
	...
	if (on) {
		...
		if (ret < 0) {
			pr_err("[USBH] %s: Unable to set HPM of the regulator:"
				"HSUSB_1p8\n", __func__);
			return ret;
		}
		...
		if (ret) {
			pr_err("[USBH] %s: unable to enable the hsusb 1p8\n",
				__func__);
		...
		if (ret) {
			pr_err("[USBH] %s: unable to enable the hsusb 3p3\n",
				__func__);
		...
		if (ret) {
			pr_err("[USBH] %s: unable to disable the hsusb 1p8\n",
				__func__);
			return ret;
		}
		...
		if (ret < 0)
			pr_err("[USBH] %s: Unable to set LPM of the regulator:"
				"HSUSB_1p8\n", __func__);

		ret = regulator_disable(hsusb_3p3);
		if (ret) {
			pr_err("[USBH] %s: unable to disable the hsusb 3p3\n",
				 __func__);
			return ret;
		}
...
static int ulpi_read(struct otg_transceiver *otg, u32 reg)
{
	...
	if (cnt >= ULPI_IO_TIMEOUT_USEC) {
		pr_err("[USBH] ulpi_read: timeout %08x reg: 0x%x\n",
...
static int ulpi_write(struct otg_transceiver *otg, u32 val, u32 reg)
{
	...
	if (cnt >= ULPI_IO_TIMEOUT_USEC) {
		pr_err("[USBH] ulpi_write: timeout reg: 0x%x ,val: 0x%x\n", reg, val);
		return -ETIMEDOUT;
	}
	return 0;
}
...
ssize_t otg_store_usb_phy_setting(const char *buf, size_t count)
{
	...
	if (i < 0) {
		pr_err("[USBH] %s: value %d\n", __func__, i);
		return 0;
	}
	pr_info("[USBH] Set 0x%02lx = 0x%02lx\n", reg, value);
...
static void ulpi_init(struct msm_otg *motg)
{
	...
	while (seq[0] >= 0) {
		pr_info("[USBH] ulpi: write 0x%02x to 0x%02x\n",
				seq[0], seq[1]);
...
static int msm_otg_link_clk_reset(struct msm_otg *motg, bool assert)
{
	...
	if (assert) {
		...
		if (ret)
			pr_err("[USBH] usb hs_clk assert failed\n");
	} else {
		ret = clk_reset(motg->clk, CLK_RESET_DEASSERT);
		if (ret)
			pr_err("[USBH] usb hs_clk deassert failed\n");
	}
	return ret;
}
...
static int msm_otg_phy_clk_reset(struct msm_otg *motg)
{
	...
	if (ret) {
		pr_err("[USBH] usb phy clk assert failed\n");
		return ret;
	}
	usleep_range(10000, 12000);
	ret = clk_reset(motg->phy_reset_clk, CLK_RESET_DEASSERT);
	if (ret)
		pr_err("[USBH] usb phy clk deassert failed\n");
	return ret;
}
...
static int msm_otg_phy_reset(struct msm_otg *motg)
{
	...
	pr_info("[USBH] phy_reset: success\n");
	return 0;
}
...
static int msm_otg_suspend(struct msm_otg *motg)
{
	...
	if (cnt >= PHY_SUSPEND_TIMEOUT_USEC) {
		pr_err("[USBH] Unable to suspend PHY\n");
		msm_otg_reset(otg);
		enable_irq(motg->irq);
		return -ETIMEDOUT;
	}
	...
	if (ret)
		pr_err("[USBH] %s failed to devote for"
			"PXO buffer%d\n", __func__, ret);
	...
	pr_info("[USBH] USB in low power mode\n");
...
static int msm_otg_resume(struct msm_otg *motg)
{
	...
	if (ret)
		pr_err("[USBH] %s failed to vote for"
			"PXO buffer%d\n", __func__, ret);
	...
	if (cnt >= PHY_RESUME_TIMEOUT_USEC) {
		 ...
		 */
		pr_err("[USBH] Unable to resume USB."
				"Re-plugin the cable\n");
		msm_otg_reset(otg);
	}
...
skip_phy_resume:
	...
	pr_info("[USBH] USB exited from low power mode\n");
...
static void msm_otg_notify_charger(struct msm_otg *motg, unsigned mA)
{
	...
	pr_info("[USBH] Avail curr from USB = %u\n", mA);
...
static int msm_otg_set_host(struct otg_transceiver *otg, struct usb_bus *host)
{
	...
	if (motg->pdata->mode == USB_PERIPHERAL) {
		pr_debug("[USBH] Host mode is not supported\n");
		return -ENODEV;
	}
	...
	otg->host = host;
	pr_debug("[USBH] host driver registered w/ tranceiver\n");
			...
			struct usb_gadget *gadget)
{
	...
	if (motg->pdata->mode == USB_HOST) {
		pr_err("[USBH] Peripheral mode is not supported\n");
		return -ENODEV;
	}
	...
	otg->gadget = gadget;
	pr_debug("[USBH] peripheral driver registered w/ tranceiver\n");
...
static bool msm_chg_aca_detect(struct msm_otg *motg)
{
	...
	default:
		...
		if (ret) {
			pr_debug("[USBH] ID A/B/C/GND is no more\n");
			motg->chg_type = USB_INVALID_CHARGER;
			motg->chg_state = USB_CHG_STATE_UNDEFINED;
		}
	}
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	pr_info("[USBH] %s: state:%s bit:0x%08x\n", __func__,
...
static irqreturn_t msm_otg_irq(int irq, void *data)
{
	...
	if ((otgsc & OTGSC_IDIS) && (otgsc & OTGSC_IDIE)) {
		...
		if (motg->vbus_notification_cb) {
			if (otgsc & OTGSC_BSV) {
				pr_debug("[USBH] BSV set\n");
				motg->vbus_notification_cb(1);
			} else {
				pr_debug("[USBH] BSV clear\n");
...
static void ac_detect_expired(unsigned long _data)
{
	...
	pr_info("[USBH] %s: count = %d, connect_type = %d\n", __func__,
...
static void msm_otg_set_id_state(int id)
{
	...
	if (!motg) {
		pr_info("[USBH] OTG does not probe yet\n");
		return;
	}
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
	...
	if (motg->usb_wq == 0) {
		pr_err("[USB] fail to create workqueue\n");
		goto free_ldo_init;
	}
	...
	if (IS_ERR(motg->xo_handle)) {
		pr_err("[USBH]  %s not able to get the handle"
	...
	ret = msm_xo_mode_vote(motg->xo_handle, MSM_XO_MODE_ON);
	if (ret) {
		pr_err("[USBH] %s failed to vote for PXO"
			"buffer%d\n", __func__, ret);
